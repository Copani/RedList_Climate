...
for f, file_name in enumerate(files[:-1]):
	...
	interm_s=calc_month_stat(T,dates,'std')
	...
	if f==0:
		...
		Tstd=np.concatenate((Tstd,interm_s),axis=2)
...
for i, ID in enumerate(LPcoords.index):
	...
	Tmean_l+=list(np.mean(Tmean[lati,loni,:,:],axis=(0,1,3)).flatten())
	Tstd_l+=list(np.mean(Tstd[lati,loni,:,:],axis=(0,1,3)).flatten())
...
data.to_csv('climate_merged/livingplanet_ERA5_T_basic.csv')
