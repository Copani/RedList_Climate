...
for q in range(4):
	if q==0:
		percentiles=np.load('threshs/' + varn + '_threshs_1940-1970_1_quart_' + str(q) + '.npy')
	else:
		percentiles=np.concatenate((percentiles,np.load('threshs/' + varn + '_threshs_1940-1970_1_quart_' + str(q) + '.npy')),axis=-1)
...
for f, file_name in enumerate(files):
	...
	#convert precipitation from m per hour to mm per day, or tas from Kelvin to Centigrade
	if varn=='tas':
		T-=273.15	
	if varn=='pr':
		T*=(24*1000)
...
#merge to livingplanet population locations
...
data.to_csv('climate_merged/livingplanet_ERA5_' + varn + '_extrm.csv')
