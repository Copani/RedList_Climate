...
example_amphibian_data <- terra::vect("C:/Users/Claus/MasterThesis/RedList_Climate/Species distribution modeling/Litoria_castanea_biggest_area_cc.shp")

# Choose a specific species
specific_species <- example_amphibian_data[example_amphibian_data$sci_name == 'Litoria castanea']
...
resolution <- 0.25  # in Grad

# Create a raster in the occurrence region of the species
## !shouldn't the resolution be adapted to the environmental data?
...
nc_file <- "C:/Users/Claus/MasterThesis/RedList_Climate/ClimateVariablesOut/bioclimatic_variables_30km_1990-2020_mean.nc"
...
# selected_layers <- grep(["MAT", "TS", "MTWM", "MTCM", "AP", "PWeQ", "PDQ", "PWaQ"],layer_names, value = TRUE)
...
# > climate_data
# class       : SpatRaster
# dimensions  : 721, 1440, 9  (nrow, ncol, nlyr)
# resolution  : 0.25, 0.25  (x, y)
# extent      : -180.125, 179.875, -90.125, 90.125  (xmin, xmax, ymin, ymax)
# coord. ref. : +proj=longlat +datum=WGS84 +no_defs
# sources     : bioclimatic_variables_30km_1990-2020_mean.nc:MAT
#               bioclimatic_variables_30km_1990-2020_mean.nc:TS
#               bioclimatic_variables_30km_1990-2020_mean.nc:MTWM
#               ... and 6 more source(s)
# varnames    : MAT
#               TS
#               MTWM
#               ...
# names       : MAT, TS, MTWM, MTCM, AP, PS, ...

# plot the all variables in climate_data within the species extent
# plot(climate_data)
...
climate_data_raster <- as(climate_data, "Raster")
climate_data_stack <- stack(climate_data_raster)
...
num_points <- nrow(raster_points_spdf) 
sdm_data <- sdmData(formular= species~., train = raster_points_spdf, predictors = climate_data_stack, bg = list(n=num_points * 3, method ='gRandom'), crs=crs(climate_data_stack))
# Build and train model with sdm Data

# List of possible methods:
#'glm','gam','maxent','rf','tree','fda','mars','svm','brt'
# methods <- c('bioclim','glm','gam','maxent','brt')
methods <- c('glm','gam')
...
# Run the model
sdm_model <- sdm(species~.,data=sdm_data,methods=methods, replicatin='sub', test.precent=test_precent, n=n)

# Consider performance measures ROC for model run
roc(sdm_model)

###### 4th step: Prediction of the habitat suitability into the whole study area
pred_model <- predict(sdm_model,newdata=climate_data_stack)
#Plot 
plot(pred_model)
